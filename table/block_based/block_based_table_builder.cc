...
Status BlockBasedTableBuilder::InsertBlockInCacheHelper(
  ...
  if (block_type == BlockType::kData || block_type == BlockType::kIndex) {
    s = InsertBlockInCache<Block>(block_contents, handle, block_type, true);
  } else if (block_type == BlockType::kFilter) {
    if (rep_->filter_builder->IsBlockBased()) {
      // for block-based filter which is deprecated.
      s = InsertBlockInCache<BlockContents>(block_contents, handle, block_type, true);
...
Status BlockBasedTableBuilder::InsertBlockInCache(const Slice& block_contents,
  ...
  if (block_cache != nullptr) {
    ...
    s = block_cache->Insert(
        ...
        nullptr, high_prio ? Cache::Priority::HIGH : Cache::Priority::LOW);
